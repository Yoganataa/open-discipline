...
export function classifyIdentifier(id:string,c:NamingDisciplineConfig):NamingFinding|null{if(!c.enabled||c.allow.identifiers.includes(id)||testPatterns(id,c.allow.patterns))return null;const m=matchBrand(id,c.brands,c.caseSensitive);if(!m)return null;const{brand,remainder}=m;if(testPatterns(id,c.block.patterns))return{identifier:id,severity:"block",brand,remainder,reason:`\`${id}\` matches a configured block pattern for brand "${brand}".`,suggestions:suggest(remainder,c)};if(testPatterns(id,c.warn.patterns))return{identifier:id,severity:"warn",brand,remainder,reason:`\`${id}\` matches a configured warn pattern for brand "${brand}".`,suggestions:suggest(remainder,c)};if(!remainder)return{identifier:id,severity:"warn",brand,remainder,reason:`\`${id}\` is the bare brand name and is ambiguous as an internal symbol.`,suggestions:[]};if(c.allow.suffixes.includes(remainder))return null;if(c.warn.ambiguousSuffixes.includes(remainder))return{identifier:id,severity:"warn",brand,remainder,reason:`\`${id}\` combines a brand with a generic architectural role ("${remainder}"). Use a semantic qualifier unless the brand is an intentional boundary identifier.`,suggestions:suggest(remainder,c)};return{identifier:id,severity:"block",brand,remainder,reason:`\`${id}\` uses a product/company/project prefix without adding domain meaning. Internal names should describe responsibility rather than ownership.`,suggestions:suggest(remainder,c)};}
function format(findings:NamingFinding[],mode:NamingDisciplineConfig["mode"]){const l=["Open Discipline — naming policy",""];for(const f of findings){l.push(`${f.severity.toUpperCase()}: ${f.reason}`);if(f.suggestions.length)l.push(`  Prefer: ${f.suggestions.join(", ")}`);}l.push("","Branding is allowed at legitimate external or user-facing boundaries.","Use an explicit allow rule for legitimate exceptions.");if(mode==="advisory")l.push("Advisory mode: the operation is not blocked.");return l.join("\n");}
