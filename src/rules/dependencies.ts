...
export const dependencyTruthRule:DisciplineRule={id:"dependency-truth",check(ctx:RuleContext):RuleFinding[]{
 ...
 for(const spec of extractImports(ctx.filePath,ctx.addedText)){const name=dependencyName(ctx.filePath,spec);if(!name)continue;
  ...
  findings.push({rule:"dependency-truth",severity:"warn",message:"Imported external dependency \"" + name + "\" is not declared in the detected project manifest. Verify the package name/API and update the correct manifest if it is genuinely required."});
 } return findings;}};