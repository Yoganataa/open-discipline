...
const CORE_INTEGRITY_PATHS=["discipline.config.json","src/index.ts","src/config.ts","src/core/**","src/rules/**","src/scanners/**",".opencode/plugins/open-disipline.ts",".opencode/plugins/**",".git/**"];
...
export const OpenDisipline:Plugin=async({directory,client})=>{
 ...
 const commandPatterns=config.commandGuards.flatMap(source=>{try{return[{source,regex:new RegExp(source)}];}catch{console.warn("[open-disipline] Invalid command guard skipped: "+source);return[];}});
 ...
 return {
  ...
  "tool.execute.before":async(input,output)=>{
   ...
   if(SHELL_TOOLS.has(input.tool)){
    ...
    if(destructive){const message="[open-disipline] "+destructive.severity.toUpperCase()+": "+destructive.message;if(destructive.severity==="block"&&config.mode==="strict")throw new Error(message);console.warn(message);}
    for(const guard of commandPatterns){guard.regex.lastIndex=0;if(!guard.regex.test(command))continue;const message="[open-disipline] "+(config.mode==="strict"?"BLOCK":"WARN")+": command matches configured guard: "+guard.source;if(config.mode==="strict")throw new Error(message);console.warn(message);}
    if(isValidationCommand(command)){const state=getState(input.sessionID);state.validationAttempted++;const key=validationKey(command);if(state.lastValidationKey===key)state.repeatedValidation++;else state.repeatedValidation=0;state.lastValidationKey=key;if(state.repeatedValidation>=2)console.warn("[open-disipline] validation-repetition: the same validation command has been attempted repeatedly. Stop looping and inspect the original failure/root cause before retrying.");}
    return;
   }
   if(input.tool==="read"&&config.readProtection.enabled){
    ...
    if(path&&matchesPath(path,config.readProtection.paths)&&!matchesPath(path,config.readProtection.allowPaths))throw new Error("[open-disipline] BLOCK: protected-file read: "+path);
    return;
   }
   ...
   if(integrityChange)throw new Error("[open-disipline] BLOCK: guardrail implementation/configuration is protected from agent modification: "+integrityChange.filePath);
  ...
  "command.execute.before":async(input)=>{
   ...
   if(destructive){const message="[open-disipline] "+destructive.severity.toUpperCase()+": "+destructive.message;if(destructive.severity==="block"&&config.mode==="strict")throw new Error(message);console.warn(message);}
   for(const guard of commandPatterns){guard.regex.lastIndex=0;if(!guard.regex.test(input.command))continue;const message="[open-disipline] "+(config.mode==="strict"?"BLOCK":"WARN")+": command matches configured guard: "+guard.source;if(config.mode==="strict")throw new Error(message);console.warn(message);}
